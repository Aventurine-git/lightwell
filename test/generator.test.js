...
test("silhouette library has a few dozen exportable motifs",()=>{assert.ok(silhouettes.length>=30);const p=makePanel({seed:"library",cells:12});for(const [kind] of silhouettes.filter(([k])=>k!=="none")){const svg=panelToSVG(p,{0:{silhouette:kind}},{motifLibrary:"charcoal-v1",motifAssets:{[kind]:"data:image/png;base64,AA=="}});assert.ok(svg.includes(`data-tile="0"`)&&svg.includes(`data-scale="1"`),kind)}});
...
test("charcoal projects never fall through to legacy vectors",()=>{const p=makePanel({cells:5});const missing=panelToSVG(p,{0:{silhouette:"leaf"}},{motifLibrary:"charcoal-v1",motifAssets:{}});assert.match(missing,/data-missing-motif="leaf"/);assert.doesNotMatch(missing,/M446 945C160 770/);const legacy=panelToSVG(p,{0:{silhouette:"leaf"}},{motifLibrary:"legacy-vector-v1"});assert.match(legacy,/M446 945C160 770/)});
